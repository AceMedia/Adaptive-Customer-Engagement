	...
	public static function defaults(): array {
		return array(
			...
			'ai_agent'       => array(
				...
				'frontend_voice_provider'    => 'auto',
				'frontend_chat_placement'    => 'bottom-right',
				'frontend_chat_dock_enabled' => true,
				'frontend_add_to_cart_enabled' => true,
	...
	public static function sanitize( array $settings ): array {
		...
		return array(
			...
			'enrichment'     => array(
				...
				'peeringdb_api_key'       => sanitize_text_field( $enrichment['peeringdb_api_key'] ?? $defaults['enrichment']['peeringdb_api_key'] ),
				'companies_house_api_key' => sanitize_text_field( $enrichment['companies_house_api_key'] ?? $defaults['enrichment']['companies_house_api_key'] ),
			),
			'amazon_connect' => array(
			...
			'ai_agent'       => array(
				...
				'public_coupon_codes'       => sanitize_textarea_field( $ai_agent['public_coupon_codes'] ?? $defaults['ai_agent']['public_coupon_codes'] ),
				...
				'max_history_messages'      => max( 1, min( 40, absint( $ai_agent['max_history_messages'] ?? $defaults['ai_agent']['max_history_messages'] ) ) ),
				...
				'frontend_voice_provider'   => in_array( sanitize_key( $ai_agent['frontend_voice_provider'] ?? 'auto' ), array( 'auto', 'browser', 'openai', 'elevenlabs' ), true ) ? sanitize_key( $ai_agent['frontend_voice_provider'] ?? 'auto' ) : 'auto',
				'frontend_chat_placement'   => in_array( sanitize_key( $ai_agent['frontend_chat_placement'] ?? 'bottom-right' ), array( 'bottom-right', 'bottom-left', 'top-right', 'top-left' ), true ) ? sanitize_key( $ai_agent['frontend_chat_placement'] ?? 'bottom-right' ) : 'bottom-right',
				'frontend_chat_dock_enabled' => rest_sanitize_boolean( $ai_agent['frontend_chat_dock_enabled'] ?? true ),
				'frontend_add_to_cart_enabled' => rest_sanitize_boolean( $ai_agent['frontend_add_to_cart_enabled'] ?? true ),
